...
### Changed
- معماری زمان‌بندی پیامک بازنویسی شد: زمان‌بندی از `woocommerce_order_status_changed` به `woocommerce_after_order_object_save` منتقل شد
- `on_status_change` فقط پیامک‌های قدیمی را لغو می‌کند و تغییر وضعیت را ذخیره می‌کند؛ `on_after_order_save` پس از اتمام کامل `save()` فراخوانی می‌شود — در این مرحله آیتم‌های سفارش حتماً در DB هستند
- فیلتر محصول (بلک لیست/وایت لیست) اکنون در زمان زمان‌بندی به درستی کار می‌کند: قانون‌های فیلترشده اصلاً وارد صف نمی‌شوند
...
### Added
- بخش «خروجی JSON قوانین» به پایین فرم قوانین زمان‌بندی اضافه شد — یک `<details>` تاشو که قوانین ذخیره‌شده را به صورت JSON نمایش می‌دهد؛ برای اشکال‌زدایی و پشتیبانی
...
### Fixed
- رفع باگ اصلی فیلتر بلک لیست/وایت لیست: در زمان ثبت سفارش جدید، hook `woocommerce_new_order` پیش از ذخیره آیتم‌ها در DB فعال می‌شود و `get_items()` آرایه خالی برمی‌گرداند — اکنون در این حالت سفارش تازه از DB بارگذاری می‌شود
- اگر بعد از reload هم آیتم‌ها موجود نبودند، فیلتر اعمال نمی‌شود و rule زمان‌بندی می‌شود تا execute-time filter (که از DB تازه بارگذاری می‌کند) آن را درست بررسی کند
